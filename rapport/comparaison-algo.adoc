...
[.text-justify]
Comme le calcul du nombre d'itérations se fait selon la longueur de la chaîne de caractères mais également des caractères constituant cette même chaîne, il semblerait que la complexité du programme ``efficacite-43.java`` soit de l'ordre de 1 + n(a(b+c)+d) avec :

____
● *n*, le nombre de caractères total ; +
● *a*, le nombre d'espaces ; +
● *b*, le nombre d'espaces *après* un espace ; +
● *c*, le nombre d'espaces *avant* un espace ; +
● Et *d*, le nombre de caractères différents de l'espace.
____
